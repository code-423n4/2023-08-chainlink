  ...
  Pausable
{
    ...
    whenActive
  {
    ...
    for (uint256 i; i < operators.length; ++i) {
      ...
      if (principal > 0) {
        ...
        // Only the operator's staked LINK amount is withdrawable after they are removed
  ...
  function getAvailableReward() public view override(IStaking) returns (uint256) {
    return i_LINK.balanceOf(address(this)) - s_pool._getTotalStakedAmount()
  ...
  function getTotalDelegatedAmount() public view override(IStaking) returns (uint256) {
    return RewardLib._getDelegatedAmount(
      s_pool.state.totalCommunityStakedAmount, i_delegationRateDenominator
    );
  }
  ...
  function onTokenTransfer(
    ...
    if (s_pool._isOperator(sender)) {
      ...
      if (s_merkleRoot != bytes32(0)) {
        ...
        if (
          !MerkleProof.verify(
            abi.decode(data, (bytes32[])), s_merkleRoot, keccak256(abi.encode(sender))
          )
        ) revert AccessForbidden();
      }
  ...
  function _stakeAsCommunityStaker(address staker, uint256 amount) private {
    ...
    s_reward._accumulateDelegationRewards(getTotalDelegatedAmount());
    uint256 extraNonDelegatedAmount =
      RewardLib._getNonDelegatedAmount(amount, i_delegationRateDenominator);
    s_reward.missed[staker].base +=
      s_reward._calculateAccruedBaseRewards(extraNonDelegatedAmount)._toUint96();
    s_reward._reserve(
      extraNonDelegatedAmount, RewardLib._getDelegatedAmount(amount, i_delegationRateDenominator)
    );
  ...
  function _exit(address staker) private returns (uint256, uint256, uint256) {
    ...
    if (stakerAccount.isOperator) {
      ...
      uint256 baseReward =
        s_reward._calculateConcludedBaseRewards(stakerAccount.stakedAmount, staker);
      uint256 delegationReward = uint256(s_reward.delegated.cumulativePerDelegate)
  ...
  function _calculateAlertingRewardAmount(
    ...
    return
      Math.min(stakedAmount / ALERTING_REWARD_STAKED_AMOUNT_DENOMINATOR, i_maxAlertingRewardAmount);
  }
