...
library RewardLib {
  ...
  struct DelegatedRewards {
    ...
    // Used to correctly account for any changes in operator count, delegated amount, or reward
    // rate.
  ...
  function _accumulateBaseRewards(Reward storage reward) internal {
    ...
    reward.base.cumulativePerMicroLINK += (
      uint256(reward.base.rate) * (cappedTimestamp - uint256(reward.base.lastAccumulateTimestamp))
    )._toUint96();
  ...
  function _accumulateDelegationRewards(Reward storage reward, uint256 delegatedAmount) internal {
    reward.delegated.cumulativePerDelegate =
      _calculateAccruedDelegatedRewards(reward, delegatedAmount)._toUint96();
  ...
  function _updateReservedRewards(
    ...
    uint96 deltaDelegatedReward =
      _calculateReward(reward, delegatedRewardAmount, duration)._toUint96();
  ...
  /// @notice calculates the amount of stake that remains after accounting for delegation
  /// requirement
  ...
  function _updateDuration(
    ...
    uint256 earnedBaseRewards =
      _getEarnedBaseRewards(reward, totalStakedAmount, totalDelegatedAmount);
    ...
    uint256 availableRewardDuration =
      (remainingRewards * REWARD_PRECISION) / (newRate * maxPoolSize);
  ...
  function _getEarnedBaseRewards(
    ...
    return reward.reserved.base
      - _calculateReward(
        reward, totalStakedAmount - totalDelegatedAmount, _getRemainingDuration(reward)
      );
  }
  ...
  function _getEarnedDelegationRewards(
    ...
    return reward.reserved.delegated
      - _calculateReward(reward, totalDelegatedAmount, _getRemainingDuration(reward));
  }
  ...
  function _slashOnFeedOperators(
    ...
    if (reward.delegated.delegatesCount == 0) return; // Skip slashing if there are no staking
      // operators

    uint256 slashableBaseRewards =
      _getSlashableBaseRewards(reward, minOperatorStakeAmount, slashableDuration);
    ...
    for (uint256 i; i < feedOperators.length; ++i) {
      ...
      slashedDelegatedAmounts[i] = _slashOperatorDelegatedRewards(
        reward, slashableDelegatedRewards, operator, totalDelegatedAmount
      );
  ...
  function _slashOperatorDelegatedRewards(
    ...
    uint256 earnedRewards =
      _getOperatorEarnedDelegatedRewards(reward, operator, totalDelegatedAmount);
    uint256 slashedRewards = Math.min(slashableRewards, earnedRewards); // max capped by earnings
  ...
  function _getOperatorEarnedBaseRewards(
    ...
    return _calculateAccruedBaseRewards(reward, operatorStakedAmount)
  ...
  function _getOperatorEarnedDelegatedRewards(
    ...
    return _calculateAccruedDelegatedRewards(reward, totalDelegatedAmount)
