...
library StakingPoolLib {
  ...
  function _setConfig(
    ...
    if (
      maxPoolSize
        < (poolState.operatorsCount * maxOperatorStakeAmount) + poolState.totalCommunityStakedAmount
    ) revert InvalidMaxStakeAmount(maxOperatorStakeAmount);
    if (pool.limits.maxPoolSize != maxPoolSize) {
  ...
  function _getTotalStakedAmount(Pool storage pool) internal view returns (uint256) {
    ...
    return
      uint256(poolState.totalCommunityStakedAmount) + uint256(poolState.totalOperatorStakedAmount);
  }
  ...
  function _addOperators(Pool storage pool, address[] calldata operators) internal {
    uint256 requiredReservedPoolSpace =
      operators.length * uint256(pool.limits.maxOperatorStakeAmount);
