...
- Unit tests can be run by running `forge test` from the root of the project directory.
- Integration tests can be run by running `FOUNDRY_PROFILE=integration forge test`.
- Invariant tests can be run by running `FOUNDRY_PROFILE=invariant forge test`.
- Gas measurements can be generated by running `FOUNDRY_PROFILE=gas forge snapshot` to update the `.gas-snapshot` file.
